...
try {
    ...
    error_log('Error in fsmil_page: ' . $e->getMessage());
...
$base_query = "FROM tbl_hm_applications_premise_food";
$params = [];
$stage_name = '';
...
if ($stage_id === 'all') {
    ...
    $base_query .= " WHERE application_current_stage IN (1,2,3,4,5,6,7,8,12,13,24)";
    $stage_name = 'Applications Under Process';
} elseif ($stage_id === 'backlog') {
    $base_query .= " WHERE application_current_stage IN (1,2,3,4,6,7,8,12,13,24) and DATEDIFF(CURDATE(), date_submitted) > 180";
...
if (isset($_GET['by'])) {
    $letter = $_GET['by'];
    if ($letter !== 'All') {
        $base_query .= ($stage_id ? " AND" : " WHERE") . " product_type LIKE :letter";
...
if ($search) {
    $base_query .= ($stage_id || isset($_GET['by']) ? " AND" : " WHERE") . " (product_type LIKE :search OR reference_no LIKE :search)";
    $params['search'] = '%' . $search . '%';
}

// Build timeline query (excludes search term)
$timeline_base_query = "FROM tbl_hm_applications_premise_food";
$timeline_params = [];
if ($stage_id === 'all') {
    // No additional WHERE clause
} elseif ($stage_id === 'under_process') {
    $timeline_base_query .= " WHERE application_current_stage IN (1,2,3,4,5,6,7,8,12,13,24)";
} elseif ($stage_id === 'backlog') {
    $timeline_base_query .= " WHERE DATEDIFF(CURDATE(), date_submitted) > 180";
} elseif ($stage_id === 'renewal') {
...
if (isset($_GET['by'])) {
    $letter = $_GET['by'];
    if ($letter !== 'All') {
        $timeline_base_query .= ($stage_id ? " AND" : " WHERE") . " product_type LIKE :letter";
        $timeline_params['letter'] = $letter . '%';
    }
}

// Fetch total rows for pagination (includes search)
...
// Fetch all applications for timeline calculations (no pagination, no search)
...
try {
    ...
    foreach ($all_applications as $application) {
        ...
        $premise_category = $application->premise_category;
    ...
    error_log('Error fetching all applications for timeline: ' . $e->getMessage());
...
// Fetch paginated applications (includes search)
...
$base_url = 'fsmil_page.php?' . http_build_query(array_filter([
    'stage_id' => $stage_id,
    'by' => $_GET['by'] ?? null,
    'limit' => $limit,
    'search' => $search
...
<head>
    <title>FSMIL - Monitoring - <?php echo htmlspecialchars($stage_name); ?></title>
 <meta charset="utf-8">
    ...
    <style>
        * {
        ...
        .well-ontime, .well-tobedelayed, .well-delayed {
            padding: 10px;
            margin-bottom: 10px;
            border-radius: 5px;
        }
        .well-ontime h4, .well-tobedelayed h4, .well-delayed h4 {
            font-size: 12px;
            margin: 0;
        }
        .well-ontime {
        ...
        <div class="dashboard-layout">
            <div class="roadmap">
                ...
                <details>
                    ...
                    <ul>
                        <li><a href="fsmil_page.php?stage_id=backlog"><i class="fas fa-history"></i> Backlog</a></li>
                        <li><a href="fsmil_page.php?stage_id=10"><i class="fas fa-box"></i> Registered Products</a></li>
                        <li><a href="fsmil_page.php?stage_id=14"><i class="fas fa-times-circle"></i> Rejected</a></li>
                        <li><a href="fsmil_page.php?stage_id=30"><i class="fas fa-calendar-times"></i> Expired</a></li>
                    </ul>
                </details>
                <details open>
                    ...
                    <ul>
                        <li>
                            <details>
                                <summary><i class="fas fa-search"></i> Assessment</summary>
                                <ul>
                                    <li><a href="fsmil_page.php?stage_id=1"><i class="fas fa-hourglass-start"></i> Pending Assessment</a></li>
                                    <li><a href="fsmil_page.php?stage_id=2"><i class="fas fa-spinner"></i> Under Assessment</a></li>
                                </ul>
                                 <li><a href="fsmil_page.php?stage_id=3"><i class="fas fa-spinner"></i> Assessed</a></li>
                                </ul>
                            </details>
                        </li>
                        <li>
                            <details>
                                ...
                                <ul>
                                    <li><a href="fsmil_page.php?stage_id=7"><i class="fas fa-hourglass-half"></i> Pending Assessment</a></li>
                                    <li><a href="fsmil_page.php?stage_id=3"><i class="fas fa-clipboard-check"></i> Under 1st Assessment</a></li>
                                    <li><a href="fsmil_page.php?stage_id=35"><i class="fas fa-clipboard-list"></i> Pending 2nd Assessment</a></li>
                                    <li><a href="fsmil_page.php?stage_id=4"><i class="fas fa-tasks"></i> Under 2nd Assessment</a></li>
                                    <li><a href="fsmil_page.php?stage_id=36"><i class="fas fa-folder-plus"></i> Pending ADD. DATA 1st Assessment</a></li>
                                    <li><a href="fsmil_page.php?stage_id=21"><i class="fas fa-file-medical"></i> ADD. DATA, Under 1st Assessment</a></li>
                                    <li><a href="fsmil_page.php?stage_id=37"><i class="fas fa-folder-plus"></i> Pending ADD. DATA 2nd Assessment</a></li>
                                    <li><a href="fsmil_page.php?stage_id=22"><i class="fas fa-file-medical"></i> ADD. DATA, Under 2nd Assessment</a></li>
                                    <li><a href="fsmil_page.php?stage_id=38"><i class="fas fa-user-tie"></i> Manager (1st & 2nd Reports Review)</a></li>
                                </ul>
                            </details>
                        </li>
                        <li>
                            <details>
                                ...
                                <ul>
                                    <li><a href="fsmil_page.php?stage_id=8"><i class="fas fa-envelope"></i> Query Letters to be Sent</a></li>
                                    <li><a href="fsmil_page.php?stage_id=25"><i class="fas fa-reply"></i> Awaiting Applicant's Feedback</a></li>
                                </ul>
                            </details>
                        </li>
                        <li>
                            <details>
                                ...
                                <ul>
                                    <li><a href="fsmil_page.php?stage_id=19"><i class="fas fa-industry"></i> Pending GMP</a></li>
                                    <li><a href="fsmil_page.php?stage_id=5"><i class="fas fa-user-check"></i> Pending Peer Review</a></li>
                                    <li><a href="fsmil_page.php?stage_id=6"><i class="fas fa-check-double"></i> Passed Peer Review</a></li>
                                </ul>
                            </details>
                        </li>
                    </ul>
            ...
            <div class="main-content">
                ...
                <div class="search-container">
                    <form id="searchForm" action="fsmil_page.php" method="GET">
                        ...
                        <button type="button" onclick="document.getElementById('searchInput').value='';document.getElementById('searchForm').submit();" style="margin-left: 10px; padding: 6px 12px; border-radius: 20px; border: 1px solid #e7eef6; background: #f8fafc; cursor: pointer;">Clear</button>
                    </form>
                </div>
                ...
                <div id="myDiv" style="width:100%; overflow: auto;">
                    <div class="table-responsive">
                        <table class="table table-striped">
                            <tr>
                                <td colspan="11"><p><a href="fsmil_dashboard.php">Back to Dashboard</a></p></td>
                            </tr>
                            <tr>
                                <td><b>No.</b></td>
                                <td><b>Reference No.</b></td>
                                <td><b>Applicant Name</b></td>
                                <td><b>Premise Category</b></td>
                            ...
                            <tbody id="myTable">
                                ...
                                foreach ($applications_req as $application_req) {
                                    ...
                                    $application_id = $application_req->application_id ;
                                    $reference_no = $application_req->reference_no;
                                    $applicant_name = $application_req->applicant_name;
                                    $premise_category = $application_req->premise_category;
                                    $product_category = $application_req->product_category;
                                    $product_type = $application_req->product_type;
                                    $province = $application_req->province;
                                    $district = $application_req->district;
                                    $sector = $application_req->sector;
                                    ...
                                    // Output table row
                                    echo "<tr class='clickable-row' data-href='fsmil_more_details.php?app_id=$application_id'>
                                        <td>" . ++$i . "</td>
                                        <td>$reference_no</td>
                                        <td>$applicant_name</td>
                                        <td>$premise_category</td>
                                        <td>$product_category</td>
                                        <td>$product_type</td>
                                        <td>$province</td>
                                        <td>$district</td>
                                         <td>$sector</td>
                                        ". ($stage_id === 'under_process' ? "<td>" . htmlspecialchars(getStageName($application_current_stage)) . "</td>" : "<td>" . $date_submitted . "</td>") . "
                                    ...
                                    if ($access == 100) {
                                        echo "<a href='update_status.php?app_id=$application_id&stage_id=$stage_id' class='btn btn-primary btn-sm'>Update RDB Status</a>";
                                    ...
                                    if ($access == 100) {
                                        echo "<a href='update_info.php?app_id=$application_id&stage_id=$stage_id' class='btn btn-primary btn-sm'>Update Info</a>";
                                   ...
                                   if ($access == 100) {
                                        echo "<a href='update_info_assignment.php?app_id=$application_id&stage_id=$stage_id' class='btn btn-primary btn-sm'>Asignments</a>";
                                    ...
                                    echo "</td>"; 
                                     ...
                                     echo "<td>";
                                    if ($access == 100) {
                                           echo "<a href='update_stages.php?app_id=$application_id&stage_id=$stage_id' class='btn btn-primary btn-sm'>Update Stages</a>";                            
                                    }
                                    echo "</td>"; 
                    ...
                    <div class="pagination-container">
                        ...
                        <div class="form-inline">
                            ...
                            <select class="form-control" onchange="window.location.href='<?php echo "fsmil_page.php?" . http_build_query(array_filter(['stage_id' => $stage_id, 'by' => $_GET['by'] ?? null, 'page' => 1, 'limit' => '', 'search' => $search], 'strlen')); ?>&limit=' + this.value">
                                <?php
